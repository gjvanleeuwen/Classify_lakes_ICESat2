        ...
        for fn in classification_df_fn_list:
            ...
            for i, (ph_start, ph_end) in enumerate(zip(start_index_array, end_index_array)):
                ...
                if classification_method == "DBSCAN":
                    eps = find_optimal_eps(data_df, min_pts=min_pts, method=eps_method,
                                           outpath=os.path.join(base_dir, 'Exploration/figures/', os.path.basename(fn)[:-4],
                                                                'histogram_EPS_ph_{}_method_{}_minpts_{}.png'.format(ph_start, eps_method, min_pts)))
                ...
                outpath = os.path.join(figures_dir, os.path.basename(fn)[:-4],
                             'photon_classification_ph_{}_{}_{}_{}.png'.format(ph_start, classification_method, eps_method, min_pts))
                plot_classified_photons(data_df, clusters, ph_start, eps, outpath)
